import { GraduationCap, User, LogOut, Zap, Timer, AlertCircle, StopCircle } from "lucide-react";
...
export default function TopAppBar({ variant = "standard", progress, currentTask, timeLeft, isLowTime, onLogoClick }: TopAppBarProps) {
  ...
  const handleEndQuiz = async () => {
    if (quiz?.id && window.confirm("Are you sure you want to end this quiz for everyone?")) {
    ...
    <header className="sticky top-0 z-50 w-full bg-surface/80 backdrop-blur-md border-b border-surface-container">
      <div className="flex justify-between items-center w-full px-6 py-4 max-w-screen-2xl mx-auto">
        <div className="flex items-center gap-4">
          <LogoLink />
          {isAuthor && quiz?.isActive && (
            <button 
              onClick={handleEndQuiz}
              className="hidden lg:flex items-center gap-2 px-4 py-2 bg-error text-on-error rounded-xl font-bold text-xs shadow-sm shadow-error/20 hover:scale-105 active:scale-95 transition-all"
            >
